/**
 * ihContentRuleSingleSource.test.ts — SOURCE-SCAN ENUMERATION TEST (IH terminal net Phase 2,
 * 2026-09-08, spec docs/superpowers/specs/2026-09-07-item-highlight-terminal-net.md §3 adversary:
 * "'Two rulebooks again.' The failure mode of this very spec. Phase 2 must MOVE the rules, not
 * copy them ... A source-scan enumeration test ... should fail if a second Item-Highlight rule
 * list appears anywhere in src/lib").
 *
 * WHAT THIS GUARDS. Phase 2 moved five deterministic Item-Highlight content rules — the
 * off-season, promo/pricing, hardcoded-capacity, third-party-brand and sentence-shape gates that
 * used to be hand-rolled inline in `validateItemHighlights` (listingPipeline.ts) — into ONE home,
 * `productDetailAttrs.ts`: the `THIRD_PARTY_BRANDS`/`THIRD_PARTY_BRAND_PHRASES` brand lists, the
 * `HIGHLIGHT_PROMO_RE`/`CAPACITY_RE` patterns, and the `ihContentRuleViolations` predicate that
 * reads them. `validateItemHighlights` now CALLS that predicate instead of re-implementing it, and
 * `capItemHighlightRepeats` (the terminal net every producer/the Regen route/the push seam already
 * call) enforces it. A SECOND definition of any of these five names anywhere else in `src/lib` is
 * exactly the "two rulebooks" regression this move exists to close — a future edit to one copy
 * (widening a brand list, tightening the promo regex) would silently NOT reach the other, and the
 * checker route and the actual SP-API push would disagree again.
 *
 ...
 * every line that DEFINES one of the five canonical names (`const`/`export const`/`function`/
 ...
 * stored text byte-for-byte) and is RED too — proven below by self-tests against synthetic trees,
 * the same "prove it bites twice" pattern `ihDoctrineSingleSource.test.ts` established, before
 * checking the real tree is clean.
 */
...
/** Matches a DEFINITION line for one of the five moved names — `const`/`function`, optionally
...
const DEFINITION_RE = /^\s*(?:export\s+)?(?:const|function)\s+(THIRD_PARTY_BRANDS|THIRD_PARTY_BRAND_PHRASES|HIGHLIGHT_PROMO_RE|CAPACITY_RE|ihContentRuleViolations)\b/

interface AllowlistEntry { file: string; line: string }

/** Every current definition of the five moved names in the live tree — verified with
 *  `rg -n "^(export )?(const|function) (THIRD_PARTY_BRANDS|THIRD_PARTY_BRAND_PHRASES|HIGHLIGHT_PROMO_RE|CAPACITY_RE|ihContentRuleViolations)\b" src/lib`
 *  (Phase 2, this session): exactly these 5 lines, all in `fba/productDetailAttrs.ts`. */
...
/** Scans `root/scanDir` for definition lines of the five moved names, returning every one that is
...
describe('IH content-rule single source (Phase 2, spec adversary "two rulebooks again"): no second definition of the five moved names under src/lib', () => {
  it('SCANNER SELF-TEST: a brand-new, unallowlisted second definition (a "second rule list") is RED', () => {
  ...
  it('SCANNER SELF-TEST: a definition matching an allowlist entry exactly is accepted', () => {
  ...
  it('the REAL tree has exactly the 5 known, canonical definitions — zero unallowlisted rule lists', () => {
  ...
  it('each of the five names is defined EXACTLY once in the real tree (the allowlist itself has no duplicate name)', () => {
    const names = ALLOWLIST.map((a) => a.line.match(DEFINITION_RE)?.[1])
    expect(new Set(names).size).toBe(names.length)
    expect(names.sort()).toEqual(['CAPACITY_RE', 'HIGHLIGHT_PROMO_RE', 'THIRD_PARTY_BRANDS', 'THIRD_PARTY_BRAND_PHRASES', 'ihContentRuleViolations'].sort())
  })
})
