...
export async function researchKeywords(
  ...
  const composition = composePool(mergeKeywordRows(nicheKeywords, competitorKeywords), seedToks, {
    // Guaranteed-stratum entry gate: a JUNK row must win a volume seat like anyone else, never a
    // guaranteed S2 one. "Junk" means a row this listing can never index (a foreign-language dupe)
    // or one that carries no design information at all (a category-generic head).
    //
    // `hasTrademark` was REMOVED from this gate (2026-08-09) — it was the second half of the same
    // mistake as the seed scrub. S2 is the stratum that GUARANTEES the design's own terms a seat
    // against the volume auction, and a design whose subject IS a protected mark carries that mark
    // in its highest-intent phrases. Banning them here means precisely the design that most needs
    // the guarantee is the one denied it: its own terms fall through to S3/S4 and compete on raw
    // volume against 400K/mo generic apparel heads — the #144 failure, re-created by a gate meant to
    // prevent a different problem. Pool membership is a RESEARCH decision (what is this design's
    // market?); trademark safety is a PUBLISH decision, enforced on every generated byte in
    // listingPipeline and again in pushExecutor. listingPipeline's own CRITICAL-phrase handling
    // already anticipates marked keywords in the pool and scrubs them at placement time.
    s2Gate: (kw) => !isForeignKeyword(kw) && !isCategoryGenericOnly(kw),
  });
...
export async function selectSeeds(opts: {
  ...
  /* ── RESEARCH SEEDS ARE NOT PUBLISHED TEXT (fixed 2026-08-09) ──────────────────────────────────
   * These four seed paths used to run `scrubTrademarks` before the phrase ever reached Jungle Scout,
   * on the stated rationale "never spend a JS credit RESEARCHING a trademark". That rationale is
   * backwards on all three counts: researching a term is not publishing it, the substituted phrase
   * costs exactly the same one credit, and asking the market about a phrase NO SHOPPER TYPES is the
   * one way to guarantee the credit buys nothing.
   *
   * MEASURED CONSEQUENCE. `keywords_by_keyword_query` returns the cluster around the phrase it is
   * given. Handed a substituted phrase, it returns the cluster around a phrase with ~zero Amazon
   * search volume — so the design's own money cluster is never harvested AT ALL, and the pool is
   * then filled by the three always-on broad universes ("graphic tees for women" and its expansion).
   * That is how a listing came back from a fresh re-research holding 139K-443K generic apparel heads
   * and not one row of its own subject. No downstream layer can recover from this: POOL_STRATA can
   * only stratify rows that arrived, and the composer's own supply guarantee cannot buy back a
   * cluster it was never told about.
   *
   * WHERE THE SCRUB CORRECTLY LIVES — the PUBLISH boundary, which is already defended in depth:
   * every generated field is scrubbed in listingPipeline (title/bullets/description/backend/item
   * highlights, ~15 sites incl. the keyword-placement paths), again at the terminal `scrubPub` door,
   * and once more in pushExecutor immediately before the bytes go to Amazon. trademarkGuard.ts's own
   * header states the rule this restores: "NOT applied to the raw research/Intelligence pool (that's
   * data, not published)". The seeds carve-out was the exception that contradicted it.
   *
   * `scrubTrademarks` is still imported and still used for the VISION-niche universe seeds below —
   * those bypass the token-overlap relevance gate (`fromUniverse: true`), which is a different risk
   * with a different answer. This change is scoped to the DESIGN's own seed.
   */
  ...
  const rulesSeed = buildSeedFromTitle(canonicalTitle || listingTitle || '', g)
  ...
  try {
    ...
    if (key && identitySrc) {
      ...
      // A SEED IS A MARKET QUESTION, NOT A PUBLISHED STRING (2026-08-09). The old prompt told the
      // model to avoid protected marks and to substitute safe phrasing — so on a design whose whole
      // subject IS the marked event, the model proposed a phrase nobody searches and the harvest came
      // back with none of the design's own keywords. We must ask the market what shoppers ACTUALLY
      // type; the substitution happens later, on every byte we publish. Stated positively here so the
      // model does not re-derive the old behaviour from a bare prohibition.
      const system = `You pick Amazon SEARCH SEEDS for a print-on-demand product. A seed is a SHORT 2-4 word phrase a shopper types to find THIS product's DESIGN/NICHE, ending in a product-type word (shirt/tee/etc). Return the design's REAL theme — NEVER generic qualifiers (a year like 2026, "personalized", "custom", a size, a color, or the blank/garment brand). Use the EXACT words a shopper would type, including the real names of events, teams and franchises: this is a market-research query, never published text, and a seed nobody searches returns nothing. Do NOT soften or substitute a phrase to make it safe. Return 1-3 seeds, MOST important first, as JSON {"seeds":[...]}.
Examples:
Title: Personalized 2026 World Cup T-Shirt – Fan Tee, USA Mexico Canada Host Countries => {"seeds":["world cup shirt","usa soccer fan tee","soccer supporter tee"]}
