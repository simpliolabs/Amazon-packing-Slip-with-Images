...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data } = await (supabase as any).from('push_verification_tasks')
      .select('id, field, detail_field, expected_value, status, attempts, max_attempts, next_check_at, last_matched_count, last_total_count, last_error')
    ...
    return NextResponse.json({ pending, needs_attention, tasks: data ?? [] })
  } catch {
    return NextResponse.json({ pending: 0, needs_attention: 0, tasks: [] })
  }
}
