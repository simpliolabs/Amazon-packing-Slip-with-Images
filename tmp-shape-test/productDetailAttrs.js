...
exports.detailValueToString = detailValueToString;
exports.normalizeFieldName = normalizeFieldName;
exports.resolveDetailAttribute = resolveDetailAttribute;
exports.isPushableDetail = isPushableDetail;
exports.unpushableReason = unpushableReason;
exports.buildDetailPatchValue = buildDetailPatchValue;
exports.isWriteBlockedPreLaunch = isWriteBlockedPreLaunch;
exports.currentDetailValue = currentDetailValue;
/**
 ...
 */
function detailValueToString(v) {
    if (v == null)
        return '';
    if (typeof v === 'string')
        return v;
    if (Array.isArray(v))
        return v.map((x) => detailValueToString(x)).filter(Boolean).join(', ');
...
function resolveDetailAttribute(fieldName) {
...
function isPushableDetail(fieldName) {
...
function unpushableReason(fieldName) {
    const a = resolveDetailAttribute(fieldName);
    if (!a)
        return 'This attribute isn’t mapped to an SP-API field yet. Copy the value and paste it in Seller Central.';
 ...
 */
function buildDetailPatchValue(attr, rawValue, marketplaceId, languageTag = 'en_US') {
...
function isWriteBlockedPreLaunch(fieldName, spApiKey, now = new Date()) {
    if (now >= new Date('2026-07-27T00:00:00Z'))
        return false;
 ...
 */
function currentDetailValue(attributes, spApiKey) {
    ...
    const walk = (n, key, depth) => {
        if (valueLeaf || n == null || depth > 6 || SKIP.has(key))
            return;
        ...
        if (typeof n === 'object') {
            for (const [k, sub] of Object.entries(n))
                walk(sub, k, depth + 1);
        }
    };
    walk(first, '', 0);
