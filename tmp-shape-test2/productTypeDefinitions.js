...
exports.buildShapedDetailValueVariants = buildShapedDetailValueVariants;
exports.getAttributeSubschema = getAttributeSubschema;
...
async function listPushableSchemaAttributes(productType, opts, max = 26) {
    if (!productType)
        return [];
    try {
        const schema = await fetchProductTypeSchema(productType, opts);
        ...
        if (!props)
            return [];
        const eligible = Object.entries(props).filter(([key]) => !MENU_EXCLUDE.has(key) && !MENU_PER_VARIANT.has(key) && !key.includes('image_locator'));
        ...
        const band = (key) => (MENU_NOISE.test(key) ? 2 : MENU_SEO_PRIORITY.test(key) ? 0 : 1);
...
function analyzeDetailValueShape(attrNode) {
    ...
    const visit = (n, path, langs, arrs, depth) => {
        if (!n || typeof n !== 'object' || depth > 12)
            return;
        if (Array.isArray(n)) {
            for (const item of n)
                visit(item, path, langs, arrs, depth + 1);
            return;
        }
        const obj = n;
...
function buildShapedDetailValue(shape, rawValue, marketplaceId, languageTag = 'en_US') {
    ...
    for (let i = shape.path.length - 1; i >= 0; i--) {
        const wrapper = { [shape.path[i]]: shape.isArrayAt[i] ? [inner] : inner };
...
function buildShapedDetailValueVariants(shape, rawValue, marketplaceId, languageTag = 'en_US') {
    const out = [];
    ...
    if (shape.languageTagAt.some(Boolean)) {
        const noLang = { ...shape, languageTagAt: shape.languageTagAt.map(() => false) };
    ...
    if (shape.isArrayAt.some(Boolean)) {
        const noArr = { ...shape, isArrayAt: shape.isArrayAt.map(() => false) };
        out.push({ id: 'no-array', value: buildShapedDetailValue(noArr, rawValue, marketplaceId, languageTag) });
    }
    const seen = new Set();
    return out.filter((v) => { const k = JSON.stringify(v.value); if (seen.has(k))
        return false; seen.add(k); return true; });
}
/** The RAW subschema node for one attribute — the ?debug=2 ground-truth view (read-only). */
async function getAttributeSubschema(productType, spApiKey, opts) {
    const schema = await fetchProductTypeSchema(productType, opts);
    return schema?.properties?.[spApiKey] ?? null;
}
